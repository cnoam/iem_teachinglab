...
This project has been refactored into a modular structure with the following modules:

1. **Users Module** (`modules/users`) - Creates Databricks users based on email addresses
2. **Groups Module** (`modules/groups`) - Creates groups and manages group memberships
3. **Clusters Module** (`modules/clusters`) - Creates clusters for each group
4. **Permissions Module** (`modules/permissions`) - Assigns permissions to groups for their clusters
5. **Libraries Module** (`modules/libraries`) - Installs required libraries on clusters
6. **Post-Installation Module** (`modules/post-installation`) - Handles post-installation tasks like terminating clusters

Each module is self-contained and can be tested, versioned, and reused independently.
...
1. `terraform apply`. After it finished, check that the resources in the Databricks portal are as expected: users created, they are in the correct group, the group has correct permissions in the correct cluster. All cluster should be turned ON. <br>
...
Examine the code in `modules/libraries/main.tf`
...
## Debugging
Try adding the env var: `TF_LOG=DEBUG terraform apply`
...
For detailed documentation on each module, see the README.md file in each module's directory:
- [Users Module](modules/users/README.md)
- [Groups Module](modules/groups/README.md)
- [Clusters Module](modules/clusters/README.md)
- [Permissions Module](modules/permissions/README.md)
- [Libraries Module](modules/libraries/README.md)
- [Post-Installation Module](modules/post-installation/README.md)
...
2025-10-20
Refactored the code into a modular structure with separate modules for users, groups, clusters, permissions, libraries, and post-installation tasks.