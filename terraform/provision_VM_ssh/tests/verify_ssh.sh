...
#
# Usage:
#   cd terraform/provision_VM_ssh
#   bash tests/verify_ssh.sh
#
# Prerequisites: terraform apply has been run, id_rsa_lab.pem exists, jq is installed.
...
for TEAM in $TEAMS; do
  ...
  for i in $(seq 1 $MAX_RETRIES); do
    if ssh $SSH_OPTS vmadmin@"$IP" 'id vmadmin && id deploy' 2>/dev/null; then
      echo "  OK: vmadmin and deploy users exist."
      SUCCESS=true
      break
    fi
