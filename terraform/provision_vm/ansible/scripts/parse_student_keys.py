...
The CSV (exported from https://technionmail-my.sharepoint.com/:x:/r/personal/cnoam_technion_ac_il/_layouts/15/Doc.aspx?sourcedoc=%7B8E857C94-D665-4093-903C-B3E8FD0AF7CC%7D
 or a Google Sheet) looks like:
...
Terraform names teams ``group_NN``, and deploy-keys.yml looks up
``keys/<group_name>.pub``, so we normalize ``group01`` -> ``group_01`` and
write the key there.

This script lives in the shared ansible/ tree but writes per-course. Run it
from inside the course folder so keys land in that course's keys/ dir:

    cd courses/<course>/
    python ../../ansible/scripts/parse_student_keys.py [CSV]

Override the destination with --keys-dir if needed.
"""
...
def main() -> int:
    ...
    ap.add_argument("--keys-dir", type=Path, default=Path.cwd() / "keys",
                    help="output directory for <group>.pub files (default: ./keys)")
